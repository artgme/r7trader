...
def plot_3d(csv_path: Path, ticker: str, param_x: str, param_y: str, z_metric: str) -> None:
    ...
    'total_pnl', 'win_rate', or 'expectancy'), read straight from a saved tuning-log CSV.
    Each grid point is the best z_metric found across all values of the other tuned parameters
    for that (x, y) combination. Builds the figure but doesn't show it — call plt.show() once
    after plotting everything so none of them block in turn."""
...
def plot_2d(csv_path: Path, ticker: str, param_x: str, y_metric: str, fit_deg: int = 2) -> None:
    """2D scatter of one tuned parameter (param_x, e.g. 'vol_len', 'vol_multiplier',
    'price_move_pct', 'trail_stop_pct', 'body_ratio_threshold', 'take_profit_pct') against one
    performance metric (y_metric: 'total_pnl', 'win_rate', or 'expectancy'), read straight from
    a saved tuning-log CSV. Every grid combo is one dot (the other params vary along each column
    of dots), with the per-x mean and best lines overlaid and, if fit_deg > 0, a degree-fit_deg
    polynomial least-squares curve through the raw cloud. Builds the figure but doesn't show it
    — call plt.show() once after plotting everything."""
    if y_metric not in ('total_pnl', 'win_rate', 'expectancy'):
        raise ValueError("y_metric must be 'total_pnl', 'win_rate', or 'expectancy'")
    ...
    bests = [max(grouped[x]) for x in xs]

    fig, ax = plt.subplots(figsize=(8, 5))
    ax.scatter(x_all, y_all, s=6, alpha=0.15, color='steelblue', label=f'all combos (n={len(rows)})')
